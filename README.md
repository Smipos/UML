...
## Лабораторная работа №5. Конечный автомат UML (Диаграмма состояний)

### Задание:

Создать диаграмму состояний для какого-либо, ранее разработанного, класса или прецедента.

**Заказ** может иметь несколько состояний: 
1. Инициализация – состояние, в котором выполняются предварительные действия при создании заказа. 
2. Открыт – состояние, в котором к заказу добавляют новые пункты. Выход из состояния возможет только при отмене заказа или заполнения всех необходимых пунктов. 
3. Закрыт – состояние, в котором выписывается счет на оплату. Выход из этого состояния происходит только после выписки счета. 
4. Отменен – состояние, где удаляются все пункты заказа
